...
try {
  ...
  /* ------------------------------------------------------------
   * LOGGING AF KØB (MVP + klar til buffs)
   * Vi skriver 1 række i log_event + N rækker i log_event_cost (én pr. ressource)
   * Dette ligger INDE i samme transaktion som købet, så alt-eller-intet.
   * ------------------------------------------------------------ */
  // 1) Bestem event/subject
  $eventType   = match($type){
    'building' => 'BUILD_PURCHASE',
    'addon'    => 'ADDON_PURCHASE',
    'research' => 'RESEARCH_COMPLETE', // hvis research købes/fuldføres i ét
    default    => 'UNKNOWN'
  };
  $subjectType = $type;
  $subjectId   = $id;
  // Vi har ikke et stabilt visningsnavn i $def lige nu; brug ID som fallback.
  $subjectName = (string)($def['name'] ?? $def['DK_Navn'] ?? $subjectId);

  // 2) Payload og costs (ingen buffs endnu → base = final)
  $payload = [
    'price_mode'    => 'no-buffs-yet',
    'buffs_applied' => [],
  ];
  ...
  $costsJson = json_encode($costs, JSON_UNESCAPED_UNICODE);

  // 3) Metadata (optional): request_id + session + ip_hash
  ...
  // 4) Indsæt i log_event
  $stmtLog = $pdo->prepare("
    INSERT INTO log_event
      (user_id, event_type, subject_type, subject_id, subject_name_cached, payload_json, costs_json, request_id, session_id, ip_hash)
    VALUES
      (?,       ?,          ?,            ?,         ?,                   ?,            ?,          ?,          ?,        ?)
  ");
  $stmtLog->execute([
  ...
  // 5) Indsæt detaljer pr. ressource i log_event_cost (for rapportering senere)
  if (!empty($costList)) {
    $stmtCost = $pdo->prepare("
      INSERT INTO log_event_cost
        (log_id, res_id, amount_base, amount_final, buff_id, buff_delta, buff_percent, notes)
      VALUES
        (?,      ?,      ?,           ?,            NULL,   NULL,       NULL,         NULL)
    ");
    foreach ($costList as $c) {
      $amt = (float)$c['amount'];
      $stmtCost->execute([$logId, (string)$c['id'], $amt, $amt]);
    }
  }
$pdo->commit();
