 ...
 * Returnerer:
 *  [
 *    'happiness' => float|null, // 0..100
 *    'popularity' => float|null, // 0..100
 *    'meta' => array // debug info
 *  ]
 *
 * Defensiv, lille implementation — kan udbygges med summary-logik senere.
 */
if (!function_exists('compute_user_stats')) {
  function compute_user_stats(PDO $db = null, int $userId = 0, ?array $defs = null, ?array $state = null): array {
    ...
    try {
      // 1) Hvis der findes en summary-funktion, prøv at genbruge den
      ...
      // 2) Brug eksplicitte felter i $state hvis caller gav state
      if (is_array($state)) {
        ...
        if (!empty($out['happiness']) || !empty($out['popularity'])) {
          $out['meta']['from'] = 'state';
          // normaliser 0..1 -> 0..100 hvis nødvendigt
      ...
      // 3) Forsøg en let DB-læsning hvis $db og tabel findes (konservativ)
