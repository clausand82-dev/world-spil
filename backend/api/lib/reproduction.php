...
function repro__read_birth_rates(array $defs): array {
  ...
  foreach ($cit as $key => $node) {
    $out[$key] = repro__read_stat_from_node($node, 'birthRate');
  }
  return $out;
}
...
function repro__compute_births(array $c, array $birthRates, array $cfgBirth, float $happiness01, float $popularity01, array $ratios): int {
  ...
  return (int)max(0, floor($raw * $mHappy * $mPop * $mHealth * $mCaps));
}
function repro__compute_immigration(array $c, array $cfgImm, float $popularity01, array $ratios, float $unempRatio): int {
  ...
  return (int)max(0, floor($imm * $mPop * $mCaps * $mUnemp));
}
function repro__compute_emigration(array $c, array $cfgEmi, float $happiness01, float $popularity01, array $ratios, float $unempRatio, float $crimeRatio): int {
  ...
  return (int)max(0, floor($emi * $mDef * $mHappy * $mPop * $mUnemp * $mCrime));
}
function repro__compute_deaths_by_group(array $c, array $cfgDeath, array $ratios, float $crimeRatio, float $homelessRatio): array {
  ...
  $deaths = [
    'baby'  => (int)floor(($c['baby']  ?? 0) * ($base['baby']  /1000.0) * $mH * $mD),
    'kids'  => (int)floor(($c['kids']  ?? 0) * ($base['kids']  /1000.0) * $mH * $mD),
    'young' => (int)floor(($c['young'] ?? 0) * ($base['young'] /1000.0) * $mH * $mD),
    'old'   => (int)floor(($c['old']   ?? 0) * ($base['old']   /1000.0) * $mH * $mD * 1.2),
  ];

  $adultKeys = ['adultsUnemployed','adultsWorker','adultsPolice','adultsFire','adultsHealth','adultsGovernment','adultsPolitician','adultsHomeless'];
  $adultsTotal = repro__sum_groups($c, $adultKeys);
  $adultsDeathsAll = (int)floor($adultsTotal * ($base['adults']/1000.0) * $mH * $mD * $mC * $mHomeless);
  $remaining = $adultsDeathsAll;
  foreach ($adultKeys as $k) {
    if ($remaining <= 0) break;
    $cnt = (int)($c[$k] ?? 0);
    if ($cnt <= 0) continue;
    $take = (int)floor($adultsDeathsAll * ($cnt / max(1,$adultsTotal)));
...
function apply_citizens_reproduction_for_user(int $userId, ?array $defs=null, ?array $state=null): array {
  ...
  for ($i=0; $i<$cycles; $i++) {
    ...
    $detail['aging'] = $deltaAging['aging'] ?? [];

    // 2) Births -> baby
    $births = repro__compute_births($c, $birthRates, ($cfg['reproduction.birth'] ?? []), $happiness01, $popularity01, $ratios);
    if ($births > 0) { $c['baby'] = ($c['baby']??0) + $births; $detail['births']['total']=$births; $detail['births']['byGroup']['baby']=$births; }
    ...
    if ($imm > 0) {
      ...
      $left = $imm;
      foreach ($dist as $k=>$w) {
        $add = (int)floor($imm * ($w/$sumW));
        if ($add>0){ $c[$k]=($c[$k]??0)+$add; $left-=$add; $detail['immigration']['byGroup'][$k]=($detail['immigration']['byGroup'][$k]??0)+$add; }
      }
    ...
    if ($emi>0) {
      $weights=[]; foreach ($c as $k=>$v){ $w=(int)$v; if (in_array($k,['baby','kids'],true)) $w=(int)floor($w*0.5); $weights[$k]=max(0,$w); }
      $sumW=array_sum($weights); $left=$emi;
      if ($sumW>0){
        foreach ($weights as $k=>$w){
          if ($left<=0) break;
          $take=(int)floor($emi*($w/$sumW));
    ...
    $totalDeaths=0;
    foreach ($deaths as $k=>$d){ if ($d>0 && !empty($c[$k])){ $c[$k]=repro__clampi($c[$k]-$d); $detail['deaths']['byGroup'][$k]=$d; $totalDeaths+=$d; } }
