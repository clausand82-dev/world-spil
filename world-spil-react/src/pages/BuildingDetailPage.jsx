...
function AddonsTab({ family, defs, state, stage, baseOwned, requirementCaches, filter, onFilterChange }) {
  ...
  const childTabs = useMemo(() => {
    ...
    for (const entry of entries) {
      ...
      const ownsParent = (ownedAddons['add.' + groupName] || 0) > 0;
      if (!ownsParent) continue;
      const labelDef = addonDefs[groupName + '.l1'];
  ...
  useEffect(() => {
    if (filter !== 'all' && !childTabs.some((tab) => tab.key === filter)) {
      if (onFilterChange) onFilterChange('all');
    }
  }, [filter, childTabs, onFilterChange]);

  const effectiveFilter = (filter && childTabs.some((tab) => tab.key === filter)) ? filter : 'all';

  const visibleEntries = entries.filter((entry) => {
    if (effectiveFilter === 'all') return true;
    if (entry.groupName === effectiveFilter) return true;
    if (entry.groupName === 'main' && entry.base === effectiveFilter) return true;
    return false;
  });
      ...
      <div className="section-head">
        ...
        {childTabs.length > 0 ? (
          <div className="tabs secondary-tabs">
            <button
              type="button"
              className={'tab ' + (effectiveFilter === 'all' ? 'active' : '')}
